...
### Fixed (coded in Commits 1+1b+1c+1d, awaiting staging validation — NOT shipped)
...
- **(1d) Set-aside preserves everything it gates on.** The workout draft was meaningful enough to trigger the ownership screen but was deleted un-preserved. Quarantine now covers core + training + workout draft, verified byte-for-byte with a manifest written last, all-or-nothing; a verified failure keeps every source key. Set-aside data is recoverable (export/delete) from the signed-out login screen.
...
- **(1c) Normalization matches the real schema.** Singular GLP `compound`, GLP settings defaults, seeded built-in symptom types excluded (archived built-ins compared by label), dynamic `startDate` excluded; one normalized representation now feeds reconciliation, baselines and emptiness. Fresh installs resolve clean instead of pending forever; exports carry training.
