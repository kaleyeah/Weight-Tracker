...
### Security (required by the gate review, 2026-07-27)
- **Credentials no longer appear in process arguments.** `/proc/<pid>/cmdline` is world-readable, so every `curl -H "Authorization: <token>"` in the server test kit published a live superuser token to any local user for the life of the request. The Authorization header now goes into a 0600 curl config file, the integrity sentinel takes its token on stdin, and account passwords reach helper processes through the environment. 11 request sites plus four password sites closed; the full 172-assertion staging suite was re-run to prove the shared HTTP layer still behaves.
- **The integrity baseline is created safely** — atomically, mode 0600, refusing to follow symlinks, with permissions verified after writing — and deleted with confirmed absence after verification or rollback.
- **Content hashing is now mandatory for production integrity checks** and on by default. Byte lengths alone cannot detect a payload change that preserves length; a hashless baseline is refused unless explicitly waived.

### Added (deployment safety — V15, required by the gate review)
- **Existing-`appdata` integrity sentinel.** The gate now records `id`, `user`, `coreRev`, `trainingRev` and the UTF-8 byte lengths of `data` and `training` for every existing row *before* deployment, and requires all six unchanged afterwards — the one check that looks at the athletes' own rows rather than at schema. Payloads are measured in memory and never written anywhere; the baseline holds six scalars per row. Byte length cannot see a change that preserves length, so an optional content-hash mode closes that gap; both behaviours are demonstrated in the evidence.
- **Disposable production probe account** (`probe-account.sh`) — creates exactly one hard-coded `cf_test_*` address so the gate can prove the commit route executes, then tears it down with verified user, appdata and ledger absence.
...
- **(1g) The workout guard compares the complete value.** 1f's length+prefix "fingerprint" could miss an ordinary same-length edit (change one rep count and restore/import/logout would proceed, destroying the newer draft). The destructive context now carries the full raw workout string and compares exactly — collision-free, and simpler than hashing since the original is already in hand.
- **(1g) Recovery manifests enforce the exact component set.** The validator accepted subset manifests (e.g. `["core"]` alone), so an incomplete or corrupted set could pass the export gate. It now requires exactly {core, training, workout} — unique, no unknown keys, sizes present and integer, every component present at its recorded length — at both the phase-2 commit and export.
