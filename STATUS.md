...
- **Latest internal build:** `2026-07-26.340-pb-c1g3` (Commits 1→1g + post-verdict hardening + main merged: the Product Owner's feature builds `.332`–`.338` — fiber, CSV export rework + BOM + totals, reopen completed day, Weekly Macros card). **257 tests pass on the merged script.** Verdict-transfer confirmation for `.340` requested from the Architect (the `.339` verdict was issued before the merge).
