...
| M8 | Sync rework (design v7 contract, approved round 8) | CLIENT EVIDENCE ACCEPTED (round 19, 128 browser cases vs a modeled server; candidate uncommitted in the compound-app working copy, extracted for every release). Real-PocketBase gate NOT yet passed: the disposable-PB integration (Owner-authorized) is the current gate. Packaging, publication, production mutation, and lockdown remain gated |
