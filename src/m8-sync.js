...
/* The authority to START a push. Fail-closed lives HERE, on new work only —
   see m8CommitFence for why it must not live at dispatch.
...
/* Attach the proof when this device has one. Deliberately BEST-EFFORT, never
   refusing: m8CasCommit serves new pushes AND journal replay, and a replay is
   not new work — it re-sends a request this device already made, to learn
   whether the server applied it. Refusing to replay without the pen strands
   that journal unresolved, which is a data-loss risk of its own; the server
   answers replays from the ledger and, under enforcement, rejects a stale one
   before any mutation (the F2 arm handles that outcome). New pushes cannot
   reach here unfenced because m8HasPen() gates them. */
function m8CommitFence(body){
  if(typeof m10AuthNow!=="function")return;
  var a=m10AuthNow();
  if(!a.ok||a.local)return;
