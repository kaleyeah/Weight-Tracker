...
/* "fenceStale" was written as a terminal outcome (m8AckOutcome) but never
   ...
   adding emptyRefused beside it. Both are listed now. */
var M8_J_OUTCOMES=["conflict","auth","not-applied","fenceStale","emptyRefused"];
...
/* ---- CAS commit route client (§3; route: server/pb_hooks/cf_cas.pb.js) ----
   ...
   FOUR call paths, each classified (Architect ruling, D1 round 1, finding 3 —
   no implicit behavior based on which UI invoked it):
     1. m8Push (:~445)            NEW WORK — start-authority m8HasPen(), then the
                                  wire invariant at dispatch
     2. conflict retry (:~510)    CONTINUATION of an authorized push (same
                                  journal, fresh requestId) — wire invariant
     3. journal replay (:~585)    RECOVERY of an existing journal — wire
                                  invariant; on refusal the journal is
                                  PRESERVED byte-for-byte, never sent unfenced
     4. m8CxChooseLocal (:~800)   NEW WORK — start-authority at its own command
                                  boundary, then the wire invariant */
...
/* The authority to START new work. The wire invariant in m8CommitFence is the
   second, unconditional layer: this gate refuses cleanly before a request
   identity is minted, and the dispatch invariant refuses anything that would
   leave unfenced regardless of which path produced it.

   This deliberately does NOT re-check m10DeviceId(). An unverifiable
   installation identity already marks the installation corrupt — malformed
   stored bytes set M10.corrupt directly (m10-lease-core.js:32) and m10Boot
   sets it for any null id (:135) — and m10AuthNow() refuses on corrupt before
   it ever reaches the holder test. An extra check here looked like
   defence-in-depth but could not fire: reinstating its absence as a mutation
   left all 47 assertions green, which is exactly how dead code masquerades as
   coverage. */
function m8HasPen(){
  if(typeof m10AuthNow!=="function")return true;   /* gate absent: unchanged */
  var a=m10AuthNow();
  return !!a.ok;}                                  /* {ok:true,local:true} when signed out */
/* THE WIRE INVARIANT (Architect ruling, D1 round 1, findings 1+2): when sync
   is configured and the M10 client is present, NO training commit leaves this
   client without BOTH a fence and a matching non-null deviceId — new push and
   journal replay alike. The first cut attached proof best-effort, reasoning
   that an enforcement-ON server rejects an unfenced request anyway; the ruling
   is that Stage 1 runs while production enforcement is OFF, where an unfenced
   request is ACCEPTED and mutates data — the client must be honest on its own.
   Returns false when the proof cannot be attached; the caller refuses locally
   with {st:"fenceStale"} and each caller's arm disposes correctly: a new push
   ends its journal (definite non-execution) and keeps the work dirty; a replay
   PRESERVES its journal byte-for-byte for a later fenced attempt (finding 2:
   never dispatch it unfenced, never delete it, surface takeover). */
function m8CommitFence(body){
  ...
  return true;}
/* THE DESTRUCTIVE-EMPTY INVARIANT.
   An empty training payload NEVER replaces a non-empty one. Not with proof,
   not with a flag, not ever — because no supported action produces that
   transition.

   Owner, 2026-08-11, asked what scenario this was even for. The honest answer
   was: only one — him deliberately deleting his last routine, exercise and
   session. Asked directly, he said he never does that. So the app does not
   need to tell a real delete-everything from a failed load, and an earlier
   design that tried to (a durable intent record minted on the emptying save)
   is gone. The Architect had already found its hole: minting in the generic
   saveTraining observes a STATE, not a COMMAND, so any bug that emptied
   training and then saved would have manufactured its own authority — the
   same class of error one layer up. Rather than build a command boundary and
   a confirmation dialog for something he never does, the transition is simply
   refused. The Architect ruled the identical shape "correct product behaviour,
   stronger not weaker" for core; it is stronger here for the same reason.

   SAY THE CONSEQUENCE PLAINLY (Architect ruling 2): **Compound has no
   supported delete-all-training operation.** I first wrote that the conflict
   banner still offers a clean slate — export both copies, keep this device's —
   and that is FALSE. m8CxChooseLocal passes the non-empty server copy as the
   prior, so an empty local copy is refused there too and the conflict simply
   re-forms. The refusal is genuinely unconditional, and the honest statement
   is that the operation does not exist rather than that it exists by another
   route. The banner still does its real job: both copies are preserved, both
   are exportable, and a non-empty local copy can be kept.

   If the Owner ever wants full deletion it is a new destructive feature with
   its own reviewed command and server contract — not a hole left open here.

   This sits at the choke point every commit passes, beside m8CommitFence and
   for the same stated reason: "new push and journal replay alike". The earlier
   refusal in m8Push stays as well; it stops a pointless journal being written
   and gives the recovery banner immediately.
   ...
   (cx.serverAtEntry at cx.serverRev), which can differ from base entirely, so
   checking base there would ask the wrong question. Callers that know better
   pass it; everyone else gets the base, which is right for an ordinary push
   and for a replay. */
function m8CommitEmptyAllowed(trainingObj,priorCanon){
  if(!m8TrainingEmpty(trainingObj))return true;      /* not a destruction */
  return m8CanonEmpty(priorCanon);}                  /* nothing to destroy */
function m8CasCommit(expectedRev,trainingObj,requestId,cb,priorCanon){
  var base;try{base=pbBase();}catch(e){cb({st:"transport"});return;}
  /* priorCanon is REQUIRED (Architect ruling 6). It used to default to a fresh
     read of the base, and that default was a live bug: m8ResolveJournal
     dispatched a stored request without passing it, so a replay was judged
     against whatever the base holds NOW rather than against oldBaseCanon — the
     content the request was actually written to replace. C44 missed it because
...
/* WHAT THIS BOOT KNOWS ABOUT THE TRAINING STORE (Owner incident 2026-08-10).

   loadTraining used to swallow every failure into `catch(e){}`, so a throwing
   localStorage, truncated bytes, unparseable JSON and a genuinely empty store
   were one outcome: state.training at its empty default, and nothing recorded
   about which had happened. Three states now, so the difference survives:
   `ok` a document was read, `absent` the key is not there, `unknown` we could
   not read or could not parse it.

   The load state no longer DECIDES anything — since 484 an empty training is
   refused over a non-empty server whatever produced it (m8CasCommit) — but it
   is still the honest answer to "why", and it is what the conflict record
   carries so a later reader can tell a device that read its own store from one
   that could not.

   The history is worth keeping because two designs were wrong before this one:
     480 gated the refusal on a boolean "did this boot read the store", and
         treated an ABSENT key as a known-empty device so first-run would not
         be blocked. First-run was never blocked either way — with no base,
         m8State is "fresh"/"bootstrap" and bootstrap adopts the server copy —
         so the exemption bought nothing and cost something real: the base
         record survives eviction of the training key, so an evicted store
         looked exactly like a new device and could still erase the server.
     483 replaced the boolean with a durable delete-all intent record, minted
         in saveTraining. The Architect found the hole: saveTraining runs AFTER
         state has changed and compares the RESULT against the base, so it
         observes a state rather than a command — any bug that emptied training
         and then saved would have manufactured its own authority. The same
         class of error, one layer up.
   Both are gone. The Owner was asked what scenario the machinery was for,
   answered that he never clears out all his training, and the whole question
   disappeared with it. */
function m8State(){
...
function m8Push(){
  ...
  /* The early half of the refusal: caught HERE, before a request identity is
     minted and before a journal is written, so a device in this state leaves
     nothing half-finished behind — the same placement rule as the STRICT gate
     above. m8CasCommit is the layer that cannot be bypassed; this is the layer
     that is tidy, and it is the one that reaches him with a reason.

     The load state no longer decides anything — the transition is refused
     whatever it says — but it is still the honest ANSWER to "why", and the
     conflict record carries it so a later reader knows whether this device had
     read its own store or not. */
  if(m8TrainingEmpty(copy)&&!m8CanonEmpty(b.val.body)){
    var srvObj=null;try{srvObj=JSON.parse(b.val.body);}catch(e){}
    m8EnterConflict(srvObj,b.val.rev,
      trainingLoadState===TRAINING_LOAD_OK?"training-empty-refused"
        :("training-not-loaded-"+trainingLoadState));
    return;}
  ...
  m8CasCommit(b.val.rev,copy,reqId,function(res){
    m8Pushing=false;
    m8AckOutcome(j,copy,pc.canon,gen,res,0);},b.val.body);}
...
function m8AckOutcome(j,copy,pushedCanon,gen,res,retried,opts){
    ...
    if(retried===0&&b2.st==="ok"&&res.payload!=null){
      ...
      if(rc.ok&&rc.canon===b2.val.body){
        ...
        if(reqId2&&m8JournalAdvance(j,"intent",{expectedRev:res.serverRev,requestId:reqId2})){
          /* the retry's prior is the content the comparison just PROVED the
             server holds — rc.canon === b2.val.body — not an implicit reread
             that could change the subject of the destructive check (ruling 5) */
  ...
  if(res.st==="fenceStale"){
    ...
    return;}
  if(res.st==="emptyRefused"){
    /* Its own result (Architect ruling 7), never folded into conflict/
       fenceStale/auth/transport — those carry different execution and retry
       meanings, and a refused destruction must inherit none of them. Refused
       BEFORE the request left this device, so it is a definite non-execution:
       the journal ends, the base and the dirty marker stay, and the server
       copy is offered rather than silently dropped. Never m8Block — he must be
       able to clear this himself. */
...
function m8ResolveJournal(j,cb){
  ...
  if(age<M8_LEDGER_MS&&j.expect.requestId){
    ...
    m8CasCommit(j.expect.expectedRev,body,j.expect.requestId,function(res){
      if(res.st==="emptyRefused"){
        /* A stored emptying push, refused at dispatch. The journal is left
           exactly where it is — not advanced, not ended, not quarantined — so
           the bytes on disk are the bytes that were written, and the
      ...
      /* the journal's OWN record of what this request was written to replace —
         never a fresh read, which would judge an old request against new
         circumstances (ruling 4) */
      j.expect.oldBaseCanon);
    return;}
  fetchCompare();}
...
function m8CxChooseLocal(){
  /* Command-boundary authority (Architect ruling, D1 round 1, finding 3):
     "keep this device's copy" mints NEW mutation intent and reaches
     m8CasCommit without passing through m8Push, so the click-layer M10_GATED
     entry is UX, not the write-authority boundary — a programmatic caller or
     refactor bypasses it. Checked here, before any request identity is minted
     or journal written; refusal preserves the conflict record untouched and
     offers the same takeover path as every other refused action. */
  if(!m8HasPen()){
    if(typeof m10TakeoverSheet==="function")m10TakeoverSheet();
    return;}
