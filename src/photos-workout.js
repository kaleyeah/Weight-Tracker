...
function routineItemHTML(rt,it,idx){
  ...
  if(it.progression){h+='<label class="wl-field wl-field-full" style="margin-top:12px"><span>Rest between sets <em>min — drives the rest-timer ping</em></span><input class="wl-ri-input wl-num" data-iid="'+it.id+'" data-field="restMin" type="text" inputmode="decimal" value="'+esc(it.restMin!=null?it.restMin:"")+'" placeholder="'+esc(state.settings.restNotifyMin||"3")+'"></label>';}
...
function finishSheetHTML(){var w=state.workout;var b=woBuckets();var ff=w.finishForm||{};
  ...
  if(w.whoopFill&&w.whoopFill.fields&&w.whoopFill.fields.length){
    ...
    if(_wf.zones&&typeof whoopZoneBarHTML==="function")h+=whoopZoneBarHTML(_wf.zones);
  }
  ...
  h+='<div id="wl-zonearea" style="margin-top:12px">'+zoneAreaHTML({hr:ff.hr,zone:(ff.zone!=null?ff.zone:null)},"wof")+'</div>';
...
h+='<button class="wl-rtrow" data-act="rt:open" data-id="'+rt.id+'"><span class="wl-rtrow-info"><span class="wl-rtrow-name">'+esc(rt.name||"Untitled Routine")+'</span><span class="wl-rtrow-sub">'+nEx+' exercise'+(nEx===1?"":"s")+'</span></span><span class="wl-rtrow-last"><span class="wl-rtrow-lastlab">last done</span><span class="wl-rtrow-lastval'+(_ld?"":" none")+'">'+(_ld?esc(lastDoneLabel(_ld)):"—")+'</span></span><span class="wl-rtrow-go">›</span></button>';});h+='</div>';}
  ...
  h+='<button class="wl-btn wl-btn-ghost wl-full" style="margin-top:10px" data-act="ex:newfromwt">'+I.plus.replace("<svg","<svg width=16 height=16")+'Add New Exercise</button>';
