...
/* Without a resting HR, Compound falls back to plain % of max and its zones
   disagree with WHOOP by as much as 27 bpm — a 140 bpm effort reads Zone 3 here
   and Zone 2 there. With one set, both use Karvonen and the bounds are
   identical. That is worth saying loudly rather than leaving as a preference. */
function whoopZonesMisaligned(){
  return whoopOn()&&num(state.settings.restingHR)==null&&!!whoopRestingAvg(30);
}
function whoopRestingOfferHTML(){
  ...
  if(cur==null){
    return '<div class="wl-whoopfill" style="margin-top:10px">'+I.alert.replace("<svg","<svg width=14 height=14")+
      '<span>Your heart-rate zones don\u2019t match WHOOP\u2019s. WHOOP builds zones from your resting heart rate; without one here, Compound falls back to a plain percentage of max and reads up to <b>27 bpm</b> off. '+
      'WHOOP\u2019s median over '+a.n+' nights is <b>'+a.median+' bpm</b>. '+
      '<button class="wl-link" data-act="whoop:useresting" data-v="'+a.median+'" style="display:inline">Use it and match WHOOP</button></span></div>';
  }
...
var WH_ZONE_NAME=["Zone 0 — resting, below 40% HRR","Zone 1 — very light / active recovery (40-60% HRR)","Zone 2 — light, aerobic base (60-70%)","Zone 3 — moderate (70-80%)","Zone 4 — hard (80-90%)","Zone 5 — max effort (90-100%)"];
/* WHOOP's own app labels the lowest bucket "Zone 0 (Resting)", so it is named
   the way the athlete sees it there, not invented afresh. */
var WH_ZONE_LBL=["Z0","Z1","Z2","Z3","Z4","Z5"];
