...
(async()=>{
  ...
  const t8=await ev(()=>{
    ...
    return {n:inp.cardio.length,rows,labels,cardioIdx,liftIdx,
  ...
  test('T8 cardio is listed first, with day/type/zone/time/cals, at every zone',()=>{
    ...
    eq(t8.rows[0],['Mon','Peloton','Z1','22 min','180'],'zone 1 must not be filtered out');
    eq(t8.rows[1],['Wed','Incline walk','Z3','45 min','410']);
    eq(t8.rows[2],['Fri','Row','—','15 min','—'],'missing zone and cals print an em dash');
